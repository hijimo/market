...
import { getUserInfo, setToken, setUserInfo } from '@/utils/ls';
...
request.interceptors.response.use(
  async (response) => {
    ...
    if (data.retCode === '0001007') {
      ...
      // 登录过期后，重新登录回到当前页面。
      if (window.location.pathname === '/dp') {
        window.location.href = '/user/login2';
      } else if (window.location.pathname !== '/user/login') {
        window.location.href = '/user/login';
      }
    } else if (data.retCode === '0001006') {
      // const { loginUri, appNo } = data.data;
      // 大屏登录页
      // 登录过期后，重新登录回到当前页面。
      setToken('');
      setUserInfo('');
      if (window.location.pathname !== '/user/login2') {
        window.location.href = '/user/login2';
      }
    }
    return response;
